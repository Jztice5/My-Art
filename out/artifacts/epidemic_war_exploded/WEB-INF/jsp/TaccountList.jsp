...
<div class="x-nav">
    ...
    <a class="layui-btn layui-btn-sm" style="line-height:1.6em;margin-top:3px;float:right" href="<%=path%>/findTaccount" title="刷新">
        <i class="layui-icon" style="line-height:30px">&#xe669;</i></a>
</div>
<div class="x-body">
    <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so" action="<%=path%>/findTaccount" >
    ...
    <xblock>
        <button id="addTaccountBtn" class="layui-btn layui-btn-sm"> <i class="layui-icon">&#xe654;</i>添加 </button>
            ...
            <tr>
                  ...
                  <th>
                      <c:if test="${di.status =='01' }">
                          <button type="button" class="layui-btn layui-btn-worn layui-btn-xs">报销中</button>
                      </c:if>
                      <c:if test="${di.status=='02'}">
                          <button type="button" class="layui-btn layui-btn-normal layui-btn-xs">已报销</button>
                      </c:if>
                  </th>
                <c:if test="${sessionScope.type == '02'}">
                <td>
                    <a title="编辑" id= "updateEdit" href="<%=path%>/findTaccountById?id=${di.id}">
                ...
                <c:if test="${sessionScope.type == '04'}">
                    <td>
                        <c:if test="${di.status=='01'}">
                            <a title="审核" onclick="member_close(this,'${di.id}')" href="javascript:;">
                                <i class="layui-icon">&#xe62f;</i>
                            </a>
                        </c:if>
                    </td>
    ...
    <div class="layui-row" id="test" style="display: none;">
        <div class="layui-col-md10">
            <form class="layui-form" id="addTaccountForm">
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <select name="purseId" lay-verify="required">
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <select name="applyId"  lay-verify="required">
    ...
    <script>
        ...
        layui.use(['jquery', 'excel','form','layer','laydate'], function(){
            ...
            $("#addTaccountBtn").click(function () {
                ...
                $("#addTaccountForm")[0].reset();
                form.on('submit(formDemo)', function(data) {
                    var param=data.field;
                    $.ajax({
                        url: '<%=path%>/addTaccount',
                        type: "post",
                        data:JSON.stringify(param),
                        contentType: "application/json; charset=utf-8",
                        success:function(){
                            layer.msg('添加成功', {icon: 1, time: 3000});
                            setTimeout(function () {window.location.href='<%=path%>/findTaccount';},2000);
        ...
        function member_del(obj,id){
            layer.confirm('确认要删除吗？',function(index){
                //发异步删除数据
                $.get("<%=path%>/deleteTaccount",{"id":id},function (data) {
                    if(data =true){
        ...
        function member_close(obj,id){
            ...
            layer.confirm('确认审核通过吗？',function(index){
                //发异步
                $.get("<%=path%>/accountPut",{"id":id,status:'02'},function (data) {
                    if(data =true){
                        layer.msg('已通过!',{icon:1,time:2000});
                    }else {
                        layer.msg('报销失败!',{icon:1,time:2000});
                    }
                    setTimeout(function () {window.location.href='<%=path%>/findTaccount';},2000);
                });
            });
