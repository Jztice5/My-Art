<%@ page contentType="text/html;charset=UTF-8" language="java"  import="com.ssm.mty.po.Tuser" %>
...
<div class="x-nav">
    ...
    <a class="layui-btn layui-btn-sm" style="line-height:1.6em;margin-top:3px;float:right" href="<%=path%>/findTuser" title="刷新">
        <i class="layui-icon" style="line-height:30px">&#xe669;</i></a>
</div>
<div class="x-body">
    <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so" action="<%=path%>/findTuser" >
    ...
    <xblock>
        <button id="addTuserBtn" class="layui-btn layui-btn-sm"> <i class="layui-icon">&#xe654;</i>添加 </button>
    ...
    <table class="layui-table">
        ...
        <c:forEach items="${pageList.list}" var="di">
            <tr>
                  ...
                  <th>${di.phone}</th>
                  <th>
                      <c:if test="${di.type =='01' }">
                          <button type="button" class="layui-btn layui-btn-normal layui-btn-xs">管理员</button>
                      </c:if>
                      <c:if test="${di.type=='02'}">
                          <button type="button" class="layui-btn layui-btn-normal layui-btn-xs">采购员</button>
                      </c:if>
                      <c:if test="${di.type=='03'}">
                          <button type="button" class="layui-btn layui-btn-normal layui-btn-xs">后勤处</button>
                      </c:if>
                      <c:if test="${di.type=='04'}">
                          <button type="button" class="layui-btn layui-btn-normal layui-btn-xs">财务处</button>
                      </c:if>
                  </th>
                <td>
                    <a title="编辑" id= "updateEdit" href="<%=path%>/findTuserById?id=${di.id}">
    ...
    <div class="layui-row" id="test" style="display: none;">
        <div class="layui-col-md10">
            <form class="layui-form" id="addTuserForm">
                <div style="margin-top: 20px;"></div>
                <div class="layui-form-item">
                    <label class="layui-form-label">人员类型：</label>
                    <div class="layui-input-block">
                        <select name="type" id="type"  lay-verify="required">
                            <option value="01" selected>管理员</option>
                            <option value="02">采购员</option>
                            <option value="03">后勤处</option>
                            <option value="04">财务处</option>
                        </select>
                ...
                <div class="layui-form-item">
                    <label class="layui-form-label">用户名：</label>
                    <div class="layui-input-block">
                        <input type="text" name="username" lay-verify="username" class="layui-input" placeholder="">
                    </div>
                </div>
                <div class="layui-form-item">
    ...
    <script>
                ...
                form.on('submit(formDemo)', function(data) {
                    ...
                    $.ajax({
                        ...
                        success:function(){
                            ...
                            setTimeout(function () {window.location.href='<%=path%>/findTuser';},2000);
        ...
        function member_del(obj,id){
            layer.confirm('确认要删除吗？',function(index){
                //发异步删除数据
                $.get("<%=path%>/deleteTuser",{"id":id},function (data) {
                    if(data =true){
